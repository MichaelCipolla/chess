...
public class GeneratePositions : MonoBehaviour
{
    ...
    private void interpretFENData()
    {
        ...
        foreach (char quad in fenInput)
        {
            ...
            if (currentSprite)
            {
                currentQuad = instantiateNewPiece(currentQuad, currentRank, currentPieceDatagram, isWhite, currentSprite);
            }
        }
    }
    ...
    private int instantiateNewPieceTemp(int currentQuad, int currentRank, ChessPieceData currentPieceDataGram, bool isWhite, Sprite currentPiece) {
